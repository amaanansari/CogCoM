    ...
    def attention_fn(
        ...
        **kw_args,
        ...
        if not cross_attention:
            ...
            kw_args["output_this_layer"]["mem_cross"] = kw_args[
                "encoder_outputs"
            ]
...
def update_mems(hiddens, mems, mems_mask, mem_lens, max_memory_length):
    ...
    with torch.no_grad():
        ...
        cur_mems_mask = torch.where(
            torch.ones_like(cur_mems_mask, device=mem_lens.device).cumsum(
                dim=1
            )
            <= einops.repeat(mem_lens, "b -> b s", s=s),
            1,
            0.0,
        )
