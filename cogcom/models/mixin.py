...
class LlamaVisionExpertFCMixin(BaseMixin):
    def __init__(
        ...
        if vision_layer_range is None:
            vision_layer_range = [
                i for i in range(min(num_vision_layers, num_layers))
            ]
        ...
        self.vision_dense_h_to_4h_list = nn.ModuleDict(
            [
                ...
                for layer_id, vision_dense_h_to_4h in zip(
                    vision_layer_range, vision_dense_h_to_4h_list
                )
            ]
        )
        self.vision_dense_4h_to_h_list = nn.ModuleDict(
            [
                ...
                for layer_id, vision_dense_4h_to_h in zip(
                    vision_layer_range, vision_dense_4h_to_h_list
                )
            ]
        )
        self.vision_gate_proj = nn.ModuleDict(
            [
                ...
                for layer_id, gate_proj in zip(
                    vision_layer_range, gate_proj_list
                )
            ]
        )
    ...
    def mlp_forward(self, hidden_states, **kw_args):
        ...
        if (
            kw_args["layer_id"] in mixin_self.vision_layer_range
            and (vision_expert_mask is not None)
            and vision_expert_mask.any()
        ):
            vision_dense_h_to_4h = mixin_self.vision_dense_h_to_4h_list[
                layer_id_key
            ]
            vision_dense_4h_to_h = mixin_self.vision_dense_4h_to_h_list[
                layer_id_key
            ]
            ...
            language_intermediate_parallel = self.activation_func(
                mixin_self.gate_proj[kw_args["layer_id"]](
                    language_hidden_state
                )
            ) * self.dense_h_to_4h(language_hidden_state)
            output[~vision_expert_mask.bool()] = self.dense_4h_to_h(
                language_intermediate_parallel
            )  # language_output

            vision_hidden_state = hidden_states[vision_expert_mask.bool()]
            vision_intermediate_parallel = vision_dense_h_to_4h(
                vision_hidden_state
            )
            ...
            output[vision_expert_mask.bool()] = vision_dense_4h_to_h(
                vision_intermediate_parallel
            )  # vision_output
        else:
            intermediate_parallel = self.activation_func(
                mixin_self.gate_proj[kw_args["layer_id"]](hidden_states)
    ...
    def copy_param(self):
        with torch.no_grad():
            for i in self.vision_layer_range:
                self.vision_gate_proj[str(i)].weight.data.copy_(
                    self.gate_proj[i].weight.data
                )
                self.vision_dense_4h_to_h_list[str(i)].weight.data.copy_(
                    self.transformer.layers[i].mlp.dense_4h_to_h.weight.data
                )
                self.vision_dense_h_to_4h_list[str(i)].weight.data.copy_(
                    self.transformer.layers[i].mlp.dense_h_to_4h.weight.data
                )
...
class LlamaVisionExpertAttnMixin(BaseMixin):
        ...
        if self.use_vision_expert:
            ...
            self.vision_query_key_value_list = nn.ModuleDict(
                [
                    ...
                    for layer_id, vision_query_key_value in zip(
                        vision_layer_range, vision_query_key_value_list
                    )
                ]
            )
            self.vision_dense_list = nn.ModuleDict(
                [
                    ...
                    for layer_id, vision_dense in zip(
                        vision_layer_range, vision_dense_list
                    )
                ]
            )
    ...
    def attention_forward(self, hidden_states, mask, **kw_args):
        ...
        layer_id_key = str(int(kw_args["layer_id"]))
        if (
            mixin_self.use_vision_expert
            and kw_args["layer_id"] in mixin_self.vision_layer_range
            and (vision_expert_mask is not None)
            and vision_expert_mask.any()
        ):
            ...
            vision_query_key_value = mixin_self.vision_query_key_value_list[
                layer_id_key
            ]
            ...
            )  # language_mixed_raw_layer
            mixed_raw_layer[vision_expert_mask.bool()] = (
                vision_query_key_value(vision_hidden_states)
            )  # vision_mixed_raw_layer
        else:
        ...
        query_layer, key_layer = mixin_self.rotary_emb(
            query_layer,
            key_layer,
        ...
        if (
            ...
            parallel_size = mpu.get_model_parallel_world_size()
            target_shape = context_layer.shape[:-1] + (
                context_layer.shape[-1] * parallel_size,
            )
            output = torch.empty(
                target_shape,
                dtype=hidden_states.dtype,
            ...
            output[~vision_expert_mask.bool()] = self.dense(
                context_layer[~vision_expert_mask.bool()]
            )  # language
            output[vision_expert_mask.bool()] = vision_dense(
                context_layer[vision_expert_mask.bool()]
            )  # vision
        else:
    ...
    def copy_param(self):
        with torch.no_grad():
            for i in self.vision_layer_range:
                self.vision_query_key_value_list[str(i)].weight.data.copy_(
                    self.transformer.layers[
                        i
                    ].attention.query_key_value.weight.data
                )
                self.vision_dense_list[str(i)].weight.data.copy_(
                    self.transformer.layers[i].attention.dense.weight.data
                )
