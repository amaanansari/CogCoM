...
class XAttn(BaseMixin):
    ...
    def attention_fn(
        ...
        **kwargs,
        ...
        dropout_p = (
            0.0  # xformers does not support dropout for eva hidden size
        )
    ...
    def attention_forward(self, hidden_states, mask, **kw_args):
        ...
        ).permute(2, 0, 3, 1, 4)  # 3, B, num_heads, N, C
...
class NewLayerForward(BaseMixin):
    ...
    def layer_forward(self, hidden_states, mask, *args, **kw_args):
        ...
        if self.training and self.drop_path > 0.0:
            if mpu.get_cuda_rng_tracker is not None:
                ...
                with mpu.get_cuda_rng_tracker().fork():
                    ...
                    attention_output = (
                        random_tensor.view(-1, 1, 1) * attention_output
                    )
        ...
        # MLP.
        mlp_output = self.post_attention_layernorm(
            self.mlp(mlp_input, **kw_args)
        )
...
class EVA2CLIPModel(BaseModel):
    def __init__(self, args, transformer=None, parallel_output=True, **kwargs):
        ...
        self.add_mixin(
            "patch_embedding",
            ImagePatchEmbeddingMixin(
                args.in_channels, args.hidden_size, property
            ),
        )
    ...
    def add_model_specific_args(cls, parser):
        group = parser.add_argument_group(
            "EVA2CLIP", "EVA2CLIP Configurations"
        )
        group.add_argument(
            "--image-size", nargs="+", type=int, default=[224, 224]
        )
        group.add_argument(
            "--pre-len", type=int, default=1
        )  # [cls] by default
        group.add_argument(
