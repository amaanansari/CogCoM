...
def get_prompt(question, shot=0):
    ...
    if shot == 0:
        prompt = TEMPLATE["Prompts"]["0shot"].format(
            MANIPULATIONS=manipulation_prompt, QUESTION=question
        )
    elif shot == 1:
        ...
        demo_prompt = "Q: {question} The solving steps are: {step}".format(
            ...
            step="; ".join(
                [
                    f"Step {i + 1}: ({stp['manipulation']}, {stp['text']})"
        ...
        demo_prompt = ""
        for demo in np.random.choice(
            TEMPLATE["Demonstrations"], n, replace=False
        ):
            q, steps = demo["question"], demo["steps"]
            prompt = ""
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--question",
        type=str,
        default="How many pople are in this office room?",
    )
