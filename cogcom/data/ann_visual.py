    ...
    for rt in result:
        ...
        if onbox is not None:
            box = [
                b + off
                for b, off in zip(
                    box, [ori_size[0], ori_size[1], ori_size[0], ori_size[1]]
                )
            ]
...
def annotate_ocr(image_path, onbox, answer=None):
    ...
    for iii in range(len(ocr_res)):
        ...
        if answer:
            ...
            if (
                max(len(ocr_res_str), len(answer)) > 0
                and float(dist) / max(len(answer), len(ocr_res_str)) >= 0.5
            ):
                found = True
                break
    if found:
...
def annoatate_grounding(image_path, onbox, caption, phrases):
    ret = groundingdino.annoatate_grounding(
        image_path, onbox, caption, phrases
    )
    return ret
    ...
    if func is not None:
        try:
            ...
            if add_mnp_first:
                ...
                desc = (
                    random.choice(PREVS).format(new_func + f"({param})") + desc
                )
            if replace_post:
                sep = re.findall(r",\s+.*?return", desc)
                if len(sep) > 0:
                    desc, _ = desc.split(sep[0])
                    desc = desc + ", {conj} `{ret}`.".format(
                        conj=random.choice(CONJS), ret=ret
                    )
            # if len(sep)>0:
        ...
        for qa in ex["metadata"]:
            ...
            if qa.get("steps", None):
                for i, step in enumerate(qa["steps"]):
                    ...
                    if (
                        ptr_func.match(step)
                        and len(ptr_func.match(step).groups()) == 4
                    ):
                    ...
                    if matched_func:
                        ...
                        if pos >= 0:
                            ...
                            phrase = param[:pos]
                            if "bbx" in variable:
                                onboxes = qa["steps_returns"].get(
                                    variable, [None]
                                )
                        phrase = find_noun_phrases(phrase)[
                            0
                        ]  # use noun phrase
                        # except:
                        ...
                        if "grounding" in func:
                            for ii, onbox in enumerate(onboxes):
                                ...
                                for k, v in final_com.items():
                                    if (
                                        onbox
                                        and isinstance(v["return"], list)
                                        and onbox in v["return"]
                                    ):
                                        _, fid = k.split("--")
                                        break
                                curid = f"{fid}--{i},{ii}"
                                # final_com[f'{i}-{ii}'] = synthesize_com(func, phrase, param, variable, onbox, ret, boxes, desc)
                                final_com[curid] = synthesize_com(
                                    func,
                                    phrase,
                                    param,
                                    variable,
                            ...
                            for ii, onbox in enumerate(onboxes):
                                ocr_res, ocr_res_str, found = annotate_ocr(
                                    image_path, onbox, qa["answer"]
                                )
                            ...
                            for k, v in final_com.items():
                                if (
                                    onboxes
                                    and onboxes[0] is not None
                                    and isinstance(v["return"], list)
                                    and onboxes == v["return"]
                                ):
                                    _, fid = k.split("--")
                                    break
                            curid = f"{fid}--{i},{ii}"
