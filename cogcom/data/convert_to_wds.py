...
def make_wds_shards(
    pattern, num_sample_per_shard, num_workers, samples, map_func, **kwargs
):
...
def write_partial_samples(pattern, shard_ids, samples, map_func, kwargs):
    ...
    for shard_id, samples in zip(shard_ids, samples):
        ...
        write_samples_into_single_shard(
            pattern, shard_id, samples, map_func, kwargs, unique_id
        )
        i += 1


def write_samples_into_single_shard(
    pattern, shard_id, samples, map_func, kwargs, unique_id
):
    fname = pattern % shard_id
    print(
        f"[{datetime.datetime.now()}] start to write samples to shard {fname}"
    )
    ...
    stream.close()
    print(
        f"[{datetime.datetime.now()}] complete to write samples to shard {fname}"
    )
    return size
