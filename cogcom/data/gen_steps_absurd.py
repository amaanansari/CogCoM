...
def process_one_line(
    ...
    for qa in data["metadata"]:
        if (
            "TDIUC" in qa["unique_id"]
            and qa.get("question_type", None)
            and qa.get("question_type").lower() == "absurd"
        ):
            ...
            status, result = func.get_response(
...
if __name__ == "__main__":
    ...
    for fname in list(glob.glob(f"{save_dir}/*", recursive=True)):
        with open(fname) as f:
            finished_lines.update(
                {
                    json.loads(line)["image_path"]: json.loads(line)
                    for line in f.readlines()
                }
            )
    print(
        f"{len(finished_lines)} items are already finished previously, which will be skipped. "
    )
        ...
        if any([ds in file_name for ds in include]):
            ...
            with open(file_name, "r") as fin:
                for line in fin:
                    ...
                    if (
                        line["image_path"] in finished_lines
                    ):  # skip the already finished lines
                        skipped += 1
                        continue
    ...
    chunk_size = len(train_lines) // num_process + int(
        bool(len(train_lines) % num_process)
    )
    chunk_src = [
        train_lines[i : i + chunk_size]
