    ...
    def create_data(
        ...
        for sid, split in enumerate(self.splits):
            question_file = os.path.join(
                self.data_dir,
                f"Questions/OpenEnded_mscoco_{split}2014_questions.json",
            )
            annotaion_file = os.path.join(
                self.data_dir,
                f"Annotations/mscoco_{split}2014_annotations.json",
            )
            ...
            print(
                f"[TDIUC - {split}]: #images/#total = {count}/{len(all_data)}, #QAs/#total={item_num}/{tot_item_num}"
            )
