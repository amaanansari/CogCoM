...
class InstructData:
    def __init__(
        ...
        self.coco = {
            "train": COCO(
                "save/raw/MSCOCO/MSCOCO2014/annotations/instances_train2014.json"
            ),
            "val": COCO(
                "save/raw/MSCOCO/MSCOCO2014/annotations/instances_val2014.json"
            ),
        }
    ...
    def create_data(
        ...
        for sid, split in enumerate(self.splits):
            question_file = os.path.join(
                self.data_dir,
                split,
                f"OpenEnded_mscoco_{split}2014_questions.json",
            )
            annotaion_file = os.path.join(
                self.data_dir, split, f"mscoco_{split}2014_annotations.json"
            )
            with open(question_file) as f1, open(annotaion_file) as f2:
                questions, anns = json.load(f1), json.load(f2)

            sorted_qs = sorted(
                questions["questions"], key=lambda x: x["question_id"]
            )
            sorted_anns = sorted(
                anns["annotations"], key=lambda x: x["question_id"]
            )
            ...
            for idx in tqdm.tqdm(range(len(sorted_qs)), desc="OKVQA"):
                ...
                img_path = os.path.join(
                    self.img_dir[split],
                    f"COCO_{split}2014_{qa_info['image_id']:012d}.jpg",
                )
                if not os.path.exists(img_path):
                    drop_num += 1
                    print(f"not found {img_path}.")
                    continue
                answer, answer_list = select_answer_by_confidence(
                    qa_info["answers"]
                )
                if answer is None:
                    drop_num += 1
                ...
                if img_path not in img2ann:
                    cocoanns = self.coco[split].loadAnns(
                        self.coco[split].getAnnIds([qa_info["image_id"]])
                    )
                    ...
                    for a in cocoanns:
                        new_anns.append(
                            {
                                "cat": self.coco[split].loadCats(
                                    [a["category_id"]]
                                )[0]["name"],
                                "supercat": self.coco[split].loadCats(
                                    [a["category_id"]]
                                )[0]["supercategory"],
                                "bbox": a["bbox"],
                            }
            ...
            with open(save_path, "w") as f:
                ...
                for count, image_path in enumerate(all_data):
                    ...
                    out_dict = {
                        ...
                        "metadata": all_data[image_path],
                        "image_ann": img2ann[image_path],
                    }
                    f.write(json.dumps(out_dict) + "\n")
                    item_num += len(all_data[image_path])
            print(
                f"[OKVQA - {split}]: #images/#total = {count}/{len(all_data)}, #QAs/#total = {item_num}/{tot_item_num}"
            )
