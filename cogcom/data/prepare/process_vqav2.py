...
class InstructData:
    ...
    def create_data(
        ...
        for i, split in enumerate(self.splits):
            ...
            answers = sorted(
                answers["annotations"], key=lambda x: x["question_id"]
            )
            ...
            for q, a in tqdm.tqdm(list(zip(questions, answers)), desc="VQAv2"):
                ...
                all_samples[image_path] = all_samples.get(image_path, []) + [
                    out_dict
                ]
                tot_item_num += 1

                if image_path not in img2ann:
                    anns = self.coco[split].loadAnns(
                        self.coco[split].getAnnIds([q["image_id"]])
                    )
            ...
            with open(save_path, "w") as f:
                ...
                for i, image_path in enumerate(all_samples):
                    ...
                    out_dict = {
                        "image_path": image_path,
                        "metadata": all_samples[image_path],
                        "image_ann": img2ann[image_path],
                    }
                    f.write(json.dumps(out_dict) + "\n")
                    item_num += len(all_samples[image_path])
            print(
                f"[VQAv2 - {split}]: #images/#total = {i}/{len(all_samples)}, #QAs/#total = {item_num}/{tot_item_num}"
            )
