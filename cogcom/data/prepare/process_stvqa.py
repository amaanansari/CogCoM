...
class InstructData:
    def __init__(
        ...
        self.data_dir = {
            "train": [
                f"save/raw/ST-VQA/train_task_{i}.json" for i in range(1, 4)
            ],
            "test": [
                f"save/raw/ST-VQA/test_task_{i}.json" for i in range(1, 4)
            ],
        }
        ...
        for sid, split in enumerate(self.splits):
            ...
            for c_data in tqdm.tqdm(data):
                ...
                if c_data["unique_id"] in question_ids:
                    print(
                        f"[{split}]: find repeated question_ids, {c_data['unique_id']}"
                    )
                else:
            ...
            print(
                f"[ST-VQA - {split}]: #images/#total = {count}/{len(all_results)}, #QAs/#total={item_num}/{tot_item_num}, {drop_num} images are not found."
            )
