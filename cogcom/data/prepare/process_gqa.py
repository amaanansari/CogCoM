...
class InstructData:
    def __init__(
        ...
        self.data_dir = {
            "train": glob.glob(
                "save/raw/GQA/train_all_questions/train_all_questions_**.json"
            ),
        ...
        for sid, split in enumerate(self.splits):
            ...
            for filename in self.data_dir[split]:
                ...
                for data in tqdm.tqdm(processed_datas, desc="GQA"):
                    if (
                        ...
                        or "fullAnswer" not in data
                    ):
                        drop_num += 1
                        continue
                    image_path = os.path.join(
                        self.img_dir, data["imageId"] + ".jpg"
                    )
                    if not os.path.exists(image_path):
                        # print(f'not found: {image_path}')
                        drop_num += 1
                        continue
                    prompt, txt, full_answer = (
                        data["question"],
                        data["answer"],
                        data["fullAnswer"],
                    )
                    conversation = {
                        "unique_id": "GQA-{}-{}".format(
                            split, "%09d" % (tot_item_num)
                        ),
                        "question": prompt,
                        "answer": txt,
                    ...
                    tot_item_num += 1
                    all_results[image_path] = all_results.get(
                        image_path, []
                    ) + [conversation]
            ...
            with open(save_path, "w") as f:
                ...
                for count, image_path in enumerate(all_results):
                    ...
                    out_dict = {
                        "image_path": image_path,
                        "metadata": all_results[image_path],
                    }
                    f.write(json.dumps(out_dict) + "\n")
                    item_num += len(all_results[image_path])
            print(
                f"[GQA - {split}]: #images/#total = {count}/{len(all_results)}, #QAs/#total={item_num}/{tot_item_num} {drop_num} images are not found."
            )
