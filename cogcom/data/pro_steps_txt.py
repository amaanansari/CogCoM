        ...
        if qa.get("steps_txt", None):  # re-decompose
            ...
            for i, stp in enumerate(out_steps):
                ...
                if i == len(out_steps) - 1:
                    ...
                    if (
                        int(
                            re.match(r"Step\s+(\d+)", out_steps[i + 1]).group(
                                1
                            )
                        )
                        <= cur_idx
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--out_dir", type=str, default="save/steps_5shot_extract"
    )
    ...
    for i in range(len(chunk_src)):
        ...
        #     )
        processed, tot_valid_qa = process_multi_lines(
            chunk_src[i], f"{save_dir}/{i}.jsonl"
        )
