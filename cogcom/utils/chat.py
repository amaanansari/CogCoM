...
def process_image(
    text, text_processor, img_processor, cross_img_processor, image=None
):
    ...
    if image is None:
        assert image_path is not None, (
            "image and image_path cannot be both None."
        )
...
def chat(
    ...
    while True:  # multi-turn
        ...
        is_image_mode = (
            ...
            or (type(image) is tuple and image != (None, None, None))
        )
        if not history:
            history = []
        if is_image_mode:
        ...
        if not is_image_mode or not no_prompt:
            ...
            prompt += text_processor.history_to_prompt(
                history=None, query=query
            )
        prompt, image_position, (torch_image, pil_img, cross_image) = (
            process_image(
                prompt,
                text_processor,
                img_processor,
        ...
        if torch_image is not None:
            ...
            for k in cross_image:
                if (
                    type(cross_image[k]) is torch.Tensor
                    and cross_image[k].dtype is not torch.int
                    and cross_image[k].dtype is not torch.long
                ):
                    cross_image[k] = cross_image[k].to(
                        next(model.parameters()).dtype
                    )
        ...
        if image_position < 5:  # no image
            inputs = text_processor.tokenizer(
                [prompt], return_tensors="pt"
            ).to(model.parameters().__next__().device)["input_ids"][0]
            # pre_image = 0
        else:
            new_prompt = prompt[image_position:]
            if not torch_image or hasattr(text_processor, "no_eoi"):
                new_prompt = new_prompt.replace(
                    text_processor.tokenizer.eoi, "", 1
                )
            ...
            inputs = inputs_dic["input_ids"].to(
        ...
        seq = torch.cat(
            [
                inputs,
                torch.tensor(
                    [-1] * (max_length - len(inputs)), device=inputs.device
                ),
            ],
            dim=0,
        )
        ...
        if image_position < 5:
            ...
            inputs = {
                **{"vision_" + k: v for k, v in torch_image.items()},
                **{"cross_" + k: v for k, v in cross_image.items()},
            }
        ...
        if parse_result:
            ...
            drawn_img = parse_response(
                pil_img,
                response,
                img_processor
                if img_processor is not None
                else cross_img_processor,
                output_name=f"output_turn{turns}.png",
            )

        response, query, image, done = text_processor.process_response(
            response, pil_img
