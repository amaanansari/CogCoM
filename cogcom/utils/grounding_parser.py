...
def refine_box(box, scale, new_width, new_height):
    box = [
        min(box[0], new_width - 1),
        min(box[1], new_height - 1),
        min(box[2], new_width - 1),
        min(box[3], new_height - 1),
    ]
    box = [
        box[0] / new_width,
        box[1] / new_height,
...
def get_text_by_box(boxlist, sep=" "):
    strs = [
        f"{box[0]:03d},{box[1]:03d},{box[2]:03d},{box[3]:03d}"
        for box in boxlist
    ]
    random.shuffle(strs)
...
def point2txt(point_list, scale, new_width, new_height, sep=" "):
    ...
    for point in point_list:
        point = [
            min(round(point[0] * scale), new_width - 1),
            min(round(point[1] * scale), new_height - 1),
        ]
...
def parse_resize(img, h, w):
    ...
    # maximize scale s.t.
    scale = math.sqrt(
        totalpatch * (lpatch / img.size[1]) * (lpatch / img.size[0])
    )
    num_feasible_rows = max(
        min(math.floor(scale * img.size[1] / lpatch), totalpatch), 1
    )
    num_feasible_cols = max(
        min(math.floor(scale * img.size[0] / lpatch), totalpatch), 1
    )
...
def vis_prop(img, boxes, new_width, new_height, file_name="output.png"):
    ...
    for boxs in boxes:
        for box in boxs:
            box = [
                box[0] / 1000 * new_width,
                box[1] / 1000 * new_height,
                box[2] / 1000 * new_width,
                box[3] / 1000 * new_height,
            ]
    ...
    plt.gca().get_xaxis().set_major_formatter(
        FuncFormatter(lambda x, p: format(round(x / new_width, 3), ","))
    )
    plt.gca().get_yaxis().set_major_formatter(
        FuncFormatter(lambda x, p: format(round(x / new_height, 3), ","))
    )
    if file_name:
...
def visualize(img, boxes, scale=14, file_name="output.png"):
    ...
    for boxs in boxes:
        for box in boxs:
            ...
            ax.add_patch(rect)
    plt.gca().get_xaxis().set_major_formatter(
        FuncFormatter(lambda x, p: format(round(x / scale, 1), ","))
    )
    plt.gca().get_yaxis().set_major_formatter(
        FuncFormatter(lambda x, p: format(round(x / scale, 1), ","))
    )
