...
def chat_history_to_prompt(self, history, query, eoi_tag="<img>"):
    ...
    if history:
        prompt = history
    img_pos = (
        0 if query.find(eoi_tag) < 0 else query.find(eoi_tag)
    )  # default position of image is at begining
    ...
    if "vision_attention_mask" in kwargs and "pre_image" in kwargs:
        ...
        for i, l in enumerate(lengths):
            ...
            kwargs["attention_mask"][
                i, :, :, pre_image + l : pre_image + self.image_length
            ] = 0.0
            kwargs["attention_mask"][
                i, :, pre_image + l : pre_image + self.image_length
            ] = 0.0
...
class llama2_text_processor:
    def __init__(
        self, tokenizer, max_target_length=2048, image_length=1225, model=None
    ):
    ...
    def __call__(self, caption, prompt="", history=None, eoi_tag="<img>"):
        ...
        if len(prompt_splits) > 0:
            input_ids.extend(
                self.tokenizer.encode(
                    prompt_splits[0], add_special_tokens=False
                )
            )
            pre_image = len(input_ids)
        for tokens in prompt_splits[1:]:
            tokens_with_img = [-100] + self.tokenizer.encode(
                tokens, add_special_tokens=False
            )
            input_ids.extend(tokens_with_img)
        context_length = len(input_ids) + (len(prompt_splits) - 1) * (
            self.image_length + 1
        )
        ...
        if len(caption_splits) > 0:
            ...
            )  # the longer caption (than the tokenizer defined) may cause error
        ...
        if len(input_ids) > self.max_target_length - self.image_length - 5:
            if (
                len(input_ids)
            ) > self.max_target_length - self.image_length + 200:
                return None
            input_ids = input_ids[
                : self.max_target_length - self.image_length - 5
            ]
        ...
        for idx in image_position:
            ...
            image_rope_mask[idx - self.image_length : idx] = [
                1
            ] * self.image_length
        ...
        np_mask = np.tril(
            np.expand_dims(np.array(attention_mask), 0).repeat(
        ...
        for idx in image_position:
            labels[idx - self.image_length - 1 : idx + 1] = [-100] * (
                self.image_length + 2
            )
        ...
        for i in range(len(input_ids)):
            if image_rope_mask[i] == 0 or (
                i > 0 and image_rope_mask[i] != image_rope_mask[i - 1]
            ):
                pid += 1
        ...
        return {
            ...
            "image_rope_mask": image_rope_mask,
            "pre_image": pre_image,
            "mem_len": mem_len,
            "mems_mask": mems_mask,
        }
    ...
    def history_to_prompt(self, history, query, eoi_tag="<img>"):
        # return _history_to_prompt(self, history, query, add_eoi_first)
        return _history_to_prompt[self.tokenizer.signal_type](
            self, history, query, eoi_tag=eoi_tag
        )
...
def get_masks_and_position_ids(seq, image_logits_mask):
    ...
    position_ids = torch.tensor(
        position_ids, dtype=torch.long, device=tokens.device
    )
...
class llama2_text_processor_inference:
    ...
    def __call__(
        self, prompt="", history=None, hard_prompt=False, eoi_tag="<img>"
    ):
        prompt = prompt if isinstance(prompt, str) else str(prompt)
        prompt = self.replace_tags_with_empty(prompt)
        # history = []
    ...
    def get_func(self, inputs, **kwargs):
        get_func = partial(
            ...
            image_logits_mask=kwargs["image_rope_mask"],
        )
        return get_func
...
if __name__ == "__main__":
    ...
    tokenizer = get_tokenizer(
        outer_tokenizer=llama2_tokenizer(
            "/share/official_pretrains/hf_home/vicuna-7b-v1.5/"
        )
    )
