...
def build_instruction(
    ...
    if options:
        ...
        if use_natural == "use_natural":
            ...
            num_choices_b = [
                f"({c})" for c in num_choices
            ]  # (1), (2), (3), ..
            lower_letter_choices = [
                f"({c})" for c in string.ascii_lowercase
            ]  # (a), (b), (c), ..
            upper_letter_choices = [
                f"({c})" for c in string.ascii_uppercase
            ]  # (A), (B), (C), ..
            op_choices = [
                f"Option {c}:" for c in num_choices
            ]  # Option 1:, Option 2:, ...
            choices_list = [
                ("", num_choices, ", "),
                ("", num_choices_b, ", "),
    ...
    # modify
        ...
        instructs = [
            f"""Select objects that do not appear in any of {region_split_token.join(region)}. Select "None" if you can't find any.{options_token} {split_token.join(options)} {concise_post}""",
        ...
        instructs = [
            f"""Does \"{text}\" describes image? {options_token} {split_token.join(options)} {concise_post}""",
    ...
    # modify
        ...
        instructs = [
            f"""Select objects from the options that appear in at least one of the regions. Select "None" if you can't find it.{region_token} {region_split_token.join(region)}. {options_token} {split_token.join(options)} {concise_post}""",
        ...
        instructs = [
            f"""What are the regions contain the object \"{text}\"? {concise_post}""",
        ...
        instructs = [
            f"""Decide if \"{text}\" is the description of {region_split_token.join(region)}. {options_token} {split_token.join(options)} {concise_post}""",
        ...
        region = region_split_token.join(
            meta_data["object_regions"]["subject"]
        )
        instructs = [
        ...
        instructs = [
            f"""Given the description of an object, generate the region that contains this object. The description is: \"{text}\" {concise_post}""",
        ...
        instructs = [
            f"Generate a sentence to describe the object in the given bounding box. The description should help people to distinguish the object from other objects in the image.\n\nBounding box: {region_split_token.join(region)}",
        ...
        instructs = [
            f"Select the reason from options to explain why the image quality is bad. {options_token} {split_token.join(options)} {concise_post}",
        ...
        instructs = [
            f"""Select the text from options that best describes the image. {options_token} {split_token.join(options)} {concise_post}""",
    ...
    # image generation tasks
        ...
        given_region = region_split_token.join(
            meta_data["object_regions"]["given_region"]
        )
        instructs = [
    ...
    # ------------------------------- misc
        ...
        dial_history = [
            f"{dial_turn['q']}, {dial_turn['a']};"
            for dial_turn in meta_data["dialog_hist"]
        ]
    ...
    assert len(instructs) >= 5, (
        f"task {task} has only {len(instructs)} instructions"
    )
