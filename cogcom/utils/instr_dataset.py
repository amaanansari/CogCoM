...
def process_fn_CommonWebDataset(
    ...
    for data in src:
            ...
            try:
                if "boxes_seq" in conv and conv["boxes_seq"]:
                    for bs in conv["boxes_seq"]:
                        ...
                        boxestxt = boxes2txt(
                            {"boxes": boxes}, scale, new_width, new_height
                        )
                        conv["value"] = conv["value"].replace(
                            BOXES_PLACEHOLDER, boxestxt, 1
                        )
                if "points_seq" in conv and conv["points_seq"]:
                    for ps in conv["points_seq"]:
                        ...
                        pointstxt = point2txt(
                            points, scale, new_width, new_height
                        )
                        conv["value"] = conv["value"].replace(
                            POINTS_PLACEHOLDER, pointstxt, 1
                        )
                # Remove image tag
                for img_tag in [
                    "\n" + IMAGE_PLACEHOLDER,
                ...
                print_rank0(
                    f"Error happened in instruct_s1_dataset with {data['__url__']}"
                )
                continue
        ...
        for i in range(0, len(conversations), 2):
            ...
            img_dict_flat = {
                "vision_" + k: v for k, v in img_dict["vision"].items()
            }
