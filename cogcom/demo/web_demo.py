...
def load_model(args):
    ...
    model, model_args = CogCoMModel.from_pretrained(
        ...
        args=argparse.Namespace(
            ...
            if (torch.cuda.is_available() and args.quant is None)
            else False,
            device="cuda",
            **vars(args),
        ),
    ...
    text_processor_infer = llama2_text_processor_inference(
        tokenizer,
        args.max_length,
        model.image_length if hasattr(model, "image_length") else 0,
        model,
        False,
        english=True,
    )
...
def post(
    ...
    try:
        with torch.no_grad():
            pil_img, image_path_grounding = process_image_without_resize(
                image_prompt
            )
            response, history, ret_imgs = chat(
                ...
                if hasattr(text_processor_infer, "invalid_slices")
                else [],
                parse_result=True,
            )
        ...
        result_text.append(
            (input_text, "Timeout! Please wait a few minutes and retry.")
        )
...
def main(args):
    ...
    with gr.Blocks(css="style.css") as demo:
            ...
            with gr.Column(scale=4):
                with gr.Group():
                    ...
                    image_prompt = gr.Image(
                        type="filepath", label="Image Prompt", value=None
                    )

                with gr.Row():
                    temperature = gr.Slider(
                        maximum=1, value=0.8, minimum=0, label="Temperature"
                    )
                    top_p = gr.Slider(
                        maximum=1, value=0.4, minimum=0, label="Top P"
                    )
                    top_k = gr.Slider(
                        maximum=100, value=10, minimum=1, step=1, label="Top K"
                    )
        ...
        image_prompt.upload(
            fn=clear_fn2, inputs=clear_button, outputs=[result_text]
        )
        image_prompt.clear(
            fn=clear_fn2, inputs=clear_button, outputs=[result_text]
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--top_k", type=int, default=1, help="top k for top k sampling"
    )
    parser.add_argument(
        "--temperature",
        type=float,
    ...
    parser.add_argument(
        ...
        type=str,
        default="chat",
        choices=["chat", "vqa", "chat_old", "base"],
