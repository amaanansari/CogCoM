...
def main():
    ...
    parser.add_argument(
        "--english", action="store_true", help="only output English"
    )
    parser.add_argument(
        "--quant",
        choices=[8, 4],
    ...
    parser.add_argument(
        "--from_pretrained",
        type=str,
        default="path/to/cogcom_base/",
        help="pretrained ckpt",
    )
    parser.add_argument(
        "--local_tokenizer",
        type=str,
        default="path/to/vicuna-7b-v1.5",
        help="tokenizer path",
    )
    parser.add_argument(
        "--no_prompt",
    ...
    if not args.english:
        print(
            "欢迎使用 Visual-AnyModel-CLI ，输入图像URL或本地路径读图，继续输入内容对话，clear 重新开始，stop 终止程序"
        )
    else:
        print(
            'Welcome to Visual-AnyModel-CLI. Enter an image URL or local file path to load an image. Continue inputting text to engage in a conversation. Type "clear" to start over, or "stop" to end the program.'
        )
    with torch.no_grad():
        while True:
            history = None
