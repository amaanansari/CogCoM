...
def disable_untrainable_params(self):
    enable = [
        "linear_proj",
        "mixins.mlp.vision_gate_proj",
        "mixins.mlp.vision_dense_h_to_4h_list",
        "mixins.mlp.vision_dense_4h_to_h_list",
        "mixins.rotary.vision_query_key_value_list",
        "mixins.rotary.vision_dense_list",
    ]
    total_trainable = 0
    for n, p in self.named_parameters():
        ...
        for e in enable:
            if (
                type(e) is tuple
                and e[0].lower() in n.lower()
                and e[1].lower() in n.lower()
            ):
                flag = True
                break
...
FineTuneTrainCogCoMModel.disable_untrainable_params = (
    ...
    for i, chain in enumerate(srt_chains[1:]):
        ...
        for turnid in range(max_turns):
            ...
            if turnid < len(chain):
                ...
                mask = 1
            final_chains[turnid] = merge(
                final_chains[turnid], turn_data
...
def broadcast_auto_com(chains):
    ...
    for turnid in chains:
        if isinstance(turnid, int):
            new_chains[turnid] = {}
            for k in type2list:
                new_chains[turnid].update(
                    mpu.broadcast_data(type2list[k], chains[turnid], k)
                )
            for k in other:
                new_chains[turnid][k] = chains[turnid][k]
    new_chains["chain_mask"] = mpu.broadcast_data(
        ["chain_mask"], chains, chains["chain_mask"].dtype
    )["chain_mask"]
...
def get_batch_com(data_iterator, args, timers):
    def to_fp_bf(data_dict):
        for k in data_dict:
            if (
                type(data_dict[k]) is torch.Tensor
                and data_dict[k].dtype is not torch.int32
                and data_dict[k].dtype is not torch.long
            ):
                if args.fp16:
...
def forward_step_com(data_iterator, model, args, timers):
    ...
    while turnid in chains_b:
        ...
        mem_kv = [
            o["mem_kv"] for o in output_per_layers
        ]  # list (num_layers) of [batch, query_length, 2d]
        mems, mems_mask = update_mems(
            mem_kv, mems, mems_mask, turn_mem_lens, max_memory_length=100000
        )  # [num_layers, batch, memory_length, 2d]

        # Shift so that tokens < n predict n
        shift_labels = turn_labels[..., 1:].contiguous()
        shift_logits = lm_logits[
            ..., -1 - shift_labels.size(-1) : -1, :
        ].contiguous()
        ...
        turn_mask = einops.repeat(
            turn_mask, "i -> (i s)", s=shift_labels.size(1)
        )  # mask empty chain
            ...
            for cur_dir, _, files in os.walk(include_dirs, followlinks=True):
                for f in files:
                    ...
                    if (
                        ...
                        local_files.extend(
                            [os.path.join(cur_dir, f)] * repeat_nums
                        )
...
def create_dataset_function(
    data_processors, image_processor, text_processor, path, args
):
    path, proc_name = path
    urls = get_tar_files(path, "tar")
    print_rank0(f"[Using data path is]: {path}")
    dataset = data_processors[proc_name](
        urls, args, image_processor, text_processor
    )
    return dataset
...
if __name__ == "__main__":
    ...
    py_parser.add_argument(
        ...
        type=str,
        default="cogcom_base",
        help="version of the model you want to load",
    )
    py_parser.add_argument(
        "--ignore_pad_token_for_loss", type=bool, default=True
    )
    py_parser.add_argument(
        "--from_pretrained",
        default="",
        type=str,
        help="pretrained model you want to load",
    )
    py_parser.add_argument(
        "--local_tokenizer",
        type=str,
    ...
    model, args = FineTuneTrainCogCoMModel.from_pretrained(
        args.from_pretrained,
        args,
        overwrite_args={"model_parallel_size": args.model_parallel_size}
    ...
    if args.use_lora:
        ...
        model.get_mixin("eva").vit_model.add_mixin(
            "lora",
            LoraMixin(
                args.eva_args["num_layers"],
                args.lora_rank,
    ...
    args.train_data = [
        (path.split("#")[0], path.split("#")[1]) for path in args.train_data
    ]  # path: Dir#Processor
    ...
    model = training_main(
        args,
        model_cls=model,
        forward_step_function=forward_step_com,
        ...
        collate_fn=data_collator_com,
    )
    if args.use_lora:
        model.get_mixin("lora").merge_lora()
        model.get_mixin("eva").vit_model.get_mixin("lora").merge_lora()
        args.use_lora = False
        args.save = "checkpoints/merged_lora_cogcom{}".format(
            args.eva_args["image_size"][0]
        )
